...
struct node_identity *actuator_id;
...
void init(void)
{
    actuator_id = malloc(sizeof(struct node_identity));
    server = malloc(sizeof(sock_udp_ep_t));
    _get_ip(&actuator_id->ip_addr);
    strcpy(actuator_id->name, "act2");
    set_remote(server, "fe80::ff:fe00:e");
    actuator_init();
...
int set_actuator_name(char *name)
{
    if (strlen(name) > 10)
    {
    ...
    {
        printf("actuator name set to %s\n", name);
        strcpy(actuator_id->name, name);
    }
...
void *register_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (send_id)
        {
            puts("sending id to server");
            coap_register_put(actuator_id, REGISTER_URI);
            send_id = false;
        }
    }
    return NULL;
